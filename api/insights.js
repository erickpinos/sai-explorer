  ...
  try {
    ...
    const [
      ...
      sql`
        ...
        FROM trades
        WHERE network = ${network} AND (tx_failed = FALSE OR tx_failed IS NULL) AND base_token_symbol IS NOT NULL
      ...
      sql`
        ...
        FROM trades
        WHERE network = ${network} AND (tx_failed = FALSE OR tx_failed IS NULL) AND leverage IS NOT NULL AND leverage > 0
      `,
      ...
      sql`
        ...
        FROM trades
        WHERE network = ${network} AND (tx_failed = FALSE OR tx_failed IS NULL) AND block_ts IS NOT NULL
        GROUP BY hour
        ORDER BY trade_count DESC
        LIMIT 1
      `,
      ...
      sql`
        ...
        FROM trades
        WHERE network = ${network} AND (tx_failed = FALSE OR tx_failed IS NULL) AND realized_pnl_collateral IS NOT NULL AND realized_pnl_collateral > 0
      ...
      sql`
        SELECT trader, evm_trader, base_token_symbol, is_long,
          ...
          leverage, trade_change_type, block_ts, tx_hash, evm_tx_hash
        FROM trades
        WHERE network = ${network} AND (tx_failed = FALSE OR tx_failed IS NULL) AND realized_pnl_pct IS NOT NULL AND realized_pnl_pct > 0
          AND trade_change_type != 'position_opened'
        ORDER BY realized_pnl_pct DESC
        LIMIT 10
      `,
